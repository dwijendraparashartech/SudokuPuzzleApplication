class Api::Sudoku::SudokuController < ApplicationController
	def sudoku_data
		return render json: solve!(params[:data].to_s.gsub(/\"/,'\'').gsub(/[\[\]]/,'').gsub(", ","").gsub("nil","0"))
	end

	def solve!(board)
		number_board = board.split("").map { |number_string| number_string.to_i }
    parsed_board = number_board.each_slice(9).to_a
    empty_positions = save_empty_positions(parsed_board)
    board = solve_puzzle(parsed_board,empty_positions)
  end
